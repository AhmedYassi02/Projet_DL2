...
class DNN(DBN):
    ...
    def init_DNN(self, Q, nb_classes):
        ...
        self.list_RBM[-1].init_RBM(p=Q[-1], q=nb_classes) # q est le nombre de classes
    ...
    def entree_sortie_reseau(self, donnees_entree):
        sortie = []
        if type(donnees_entree) == np.ndarray: donnees_entree = torchnp(donnees_entree).double().to(device)
    ...
    def retropropagation(self, epochs, lr, batch_fraction, X, Y, plot_error=False):
        ...
        if type(Y) == np.ndarray: Y = torchnp(Y).double().to(device)
        ...
        couche.W = np.random.randn(int(couche.W.shape[0]), int(couche.W.shape[1]))*0.01
            ...
            for x, y in zip(X_batches, Y_batches) :
                ...
                for i in reversed(range(self.nb_couche)):
                    ...
                    if i == self.nb_couche - 1:
                        ...
                        delta_b = (delta_b @ self.list_RBM[i+1].W.T) * (sortie[i] * (1 - sortie[i]))
...
if __name__ == '__main__':
    ...
    dnn.pretrain_DNN(epochs=[100], lr=0.1, batch_fraction=0.2, x=data, plot_error=True)
